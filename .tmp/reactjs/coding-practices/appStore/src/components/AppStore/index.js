...
class AppStore extends Component {
  ...
  getActiveApps = app => {
    ...
    const filteredApps = app.filter(eachApp => eachApp.category === activeTabId)
    return filteredApps
  }

  getSearchApps = () => {
    const {searchInput} = this.state
    const searchResults = appsList.filter(searchApp =>
      searchApp.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
  ...
  render() {
    ...
    const searchResults = this.getSearchApps()
